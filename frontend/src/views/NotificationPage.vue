...
const openNotification = async (notification: Notification) => {
  if (!notification.read) {
    try {
      await axios.put(
        `https://backend.cpceventscan.com/api/notifications/mark-read/${notification.id}`,
  ...
  try {
    await Promise.all(
      notifications.value.map(n =>
        axios.put(`https://backend.cpceventscan.com/api/notifications/mark-read/${n.id}`, {
          student_id: studentId.value
        })
      )
    );
