      ...
      <div id="container">
        <div v-if="eventData">
          ...
          <div class="attendance-section">
            ...
            <div class="buttons-row">
             ...
             <ion-button
              ...
              >
                {{
                  new Date(studentAttendance?.morning_trivia_time_in).toISOString() === '1900-01-01T00:00:00.000Z'
                    ? 'Missed'
            ...
            >
              {{
                new Date(studentAttendance?.afternoon_trivia_time_in).toISOString() === '1900-01-01T00:00:00.000Z'
                  ? 'Missed'
          ...
          <ion-card>
            ...
            <ion-card-content>
              <img
                :src="`https://backend.cpceventscan.com/${eventData.event_program_attachment}`"
...
const fetchAttendanceControls = async () => {
  try {
    const res = await fetch(`https://backend.cpceventscan.com/api/attendance/controls/${eventId}`, {
      credentials: "include",
    });
...
const fetchAttendance = async () => {
  ...
  try {
    const res = await fetch(
      `https://backend.cpceventscan.com/api/attendance/${eventId}/check/`,
      { credentials: "include" }
    );
    const data = await res.json();
...
onMounted(async () => {
  try {
    const resEvents = await fetch("https://backend.cpceventscan.com/api/events/list", {
      credentials: "include",
    });
...
const handleAction = async (action) => {
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      ...
      window.location.href = `https://cpceventscan.com/face-verification?event=${eventId}&action=${action}`;
    },
    (err) => {
      Swal.fire({
        icon: "error",
...
const handleTrivia = async (period) => {
  try {
    const res = await axios.get(`https://backend.cpceventscan.com/api/trivia/${eventId}/${period}`);
    ...
    if (userAnswer === randomTrivia.correct_answer) {
      ...
      await axios.put(`https://backend.cpceventscan.com/api/attendance/mark-trivia-missed/${eventId}`, {
        studentId,
        period,
      });
...
const submitFeedback = async () => {
  ...
  try {
    const response = await axios.post("https://backend.cpceventscan.com/api/feedback/add", {
